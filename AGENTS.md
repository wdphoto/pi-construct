...
- Dashboard Right Arrow on a package row unfolds package-contained resources inline using Pi's native resolver only when multiple resources are already known; Left Arrow folds, Space on child rows changes target enabled state, and Enter previews/writes native Pi package filters after backup/re-read safeguards. Available package rows are cache-inspected without network/download during dashboard build and show the normal collapsed arrow only when multiple resources are already known; focused unknown Available rows can still use Right Arrow to ask Pi to inspect/cache resources without showing a false unfold arrow first. Single-resource and unknown-after-inspection Available packages stay whole-package rows. Resource-level selection writes an explicit allowlist across all package resource kinds so future package-added resources stay disabled until selected; whole-package row Enter is the path for trusting package defaults. `r` remains package-level project removal; package-contained child resources are filtered, not removed individually.
