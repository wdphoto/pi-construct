...
- Dashboard Right Arrow on a package row unfolds package-contained resources inline using Pi's native resolver; Left Arrow folds, Space on child rows changes target enabled state, Enter previews/writes native Pi package filters after backup/re-read safeguards, and `i` opens details. For Available package rows, child selection installs the remembered package project-local first and then writes native Pi package filters. `r` remains package-level project removal; package-contained child resources are filtered, not removed individually.
