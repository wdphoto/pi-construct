...
- Dashboard Right Arrow on an installed package row unfolds package-contained resources inline using Pi's native resolver only when multiple resources are available; Left Arrow folds, Space on child rows changes target enabled state, and Enter previews/writes native Pi package filters after backup/re-read safeguards. Available package rows show the normal collapsed arrow before resource counts are known; Right Arrow lazily inspects resources, and if multiple resources exist, child selection starts unchecked, installs the remembered package project-local first, and then writes native Pi package filters. Resource-level selection writes an explicit allowlist across all package resource kinds so future package-added resources stay disabled until selected; whole-package row Enter is the path for trusting package defaults. Packages with zero or one resolved resource remain whole-package rows and show no unfold affordance after inspection. `r` remains package-level project removal; package-contained child resources are filtered, not removed individually.
