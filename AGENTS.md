...
- Pi package entries with `autoload: false` are project resource override deltas, not ordinary project package declarations. Construct must show them read-only, exclude them from library/load/save/run/remove/toggle operations, and direct users to `pi config -l` for inherit/load/unload changes.
