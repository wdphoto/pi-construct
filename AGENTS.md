...
- Dashboard `Ctrl+U` acts only when checkboxes are selected and asks for explicit confirmation before unloading global Construct-library entries, saved-loadout references, and current-project metadata. `Ctrl+R` remains project removal with its existing focused-row fallback; `Alt+I` opens details. Plain `u`, `r`, and `i`, upper or lower case, always remain filter text, including with selected rows. Do not use `Ctrl+I`: it is legacy Tab.
