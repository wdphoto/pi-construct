...
- Dashboard Right Arrow on a **declared** package row unfolds package-contained resources inline using Pi's native resolver only when multiple resources are already known; Left Arrow folds, child state icons show current state (`✓` active, `–` inactive, `+` available), `[x]` selects a child for the next action, and parent Space cycles declared-child selection presets all → active → inactive → none. Enter previews/writes native Pi package filters after backup/re-read safeguards: selected existing child resources are toggled and unselected existing children keep their current state. Resource-level selection writes an explicit allowlist across all package resource kinds so future package-added resources stay disabled until selected. Available package rows are cache-inspected without network/download during dashboard build and may show collapsed read-only preview children from Pi's temporary cache, but those previews are informational only: they are never selectable and never build an install+filter plan. Enter on an Available package row installs the whole package with Pi's normal defaults; after reopening (before `/reload`), select the live Pi-resolved resources from the now-declared package row (or use `pi config -l`). Unknown Available rows with no cached multi-resource list have no Right Arrow action, and single-resource Available rows stay whole-package rows. Whole-package row Enter remains the path for trusting package defaults. `Ctrl+Alt+R` remains package-level project removal; package-contained child resources are filtered, not removed individually.
...
- Available install and resource filtering are separate phases: an Available row installs the whole package only, and package resource filters are written later from the live declared package after reopen. Every Install step is gated immediately before its mutation by a live current-project trust check and that the reviewed source is still undeclared; a later target that became declared or untrusted refuses honestly without undoing earlier applied steps. This shared preflight is the operation-runner default, so `/construct run` uses it too, and a preflight that throws from the optional hook becomes a structured failed step instead of escaping the generic UI and losing earlier changes. Installs never offer automatic reload: the result says Pi defaults remain unfiltered and to reopen /construct or run `pi config -l` before `/reload` to choose resources, or to `/reload` explicitly to accept defaults; print-mode `/construct run` says the same instead of claiming no reload is needed. Install uses Pi's native project-local path and keeps trust, backup, and metadata safety; do not promise rollback.
