...
### Fixed
- Refresh the bounded advisory Agent Skill inventory for an already-adopted carrier during `/construct load`, fixing the missing collapsed library arrow for a skill repository adopted before inventory tracking (for example `spf13/go-skills`). The refresh updates existing library entries only; it never creates one.
- Let an Available Agent Skills carrier without a catalog snapshot show read-only children from Pi's already-existing temporary cache. This cache-only, offline lookup never installs, fetches, scans another project, or persists data; it can be stale, so validated catalog snapshots and native Pi resources take priority.

## 0.0.32 - 2026-09-19

### Added
- Adapt declared Git Agent Skills repositories that expose zero native Pi package resources: discover valid skills in the managed checkout (including a `SKILL.md` at the package root), list them as collapsed dashboard package child rows (Right Arrow unfolds, the parent row reports the available count, and single-skill carriers stay inspectable), and link selected skill roots through the project-level top-level `skills` setting while keeping the package declared for `pi update --extensions`. Carriers classify as Active when any linked skill is enabled and Disabled otherwise (nothing linked yet or all linked paths off), with Unresolved reserved for genuinely zero-resource packages that expose no adapter; the shared save/run effective-state path maps unlinked carriers to inactive and run/save output points at `/construct` package children (separate from generic all-off/partial filters) while recipes stay source-only, and `status full` summarizes found/linked/enabled skills. Available library rows expose the same collapsed read-only skill children from a bounded advisory catalog inventory recorded during `/construct load` and successful dashboard carrier removal (never from a project checkout, and native Pi resources always win). The inventory contains only validated, capped names, descriptions, and package-relative roots, keeps a previously inspected carrier expandable after its project checkout disappears, and is never used for settings writes; dashboard removal updates existing library entries only and never creates one, and an authoritative inspection that finds no adapter (native package, malformed, or unrepresentable) clears stale inventory rather than preserving it. At that release, a source with no recorded snapshot showed no Agent Skills arrow; snapshots were recorded only by `/construct load` and successful dashboard removal of an existing library item, not by first installation, while parent Enter installed normally. Removing a carrier removes its exact and carrier-owned glob skill paths first, refuses broader patterns that could dangle, reports post-cleanup failures as partial changes with reload consent, and a malformed `.claude-plugin/marketplace.json` disables the adapter instead of publishing template skills. Carrier-owned skills are excluded from `/construct load` direct adoption and from full status's direct-resource section, and carrier removal prunes legacy duplicate direct skill metadata under the checkout without deleting files or unrelated metadata. Adapted selections are project-local like package child filters and are not stored in saved loadout recipes.
...
- Allow Available package rows to use the normal collapsed arrow for resource plucking only when a cache-only Pi inspection already finds multiple package-contained resources, while focused unknown rows can still inspect/cache on Right Arrow; selecting children installs project-local and writes native package filters.
...
- Avoid false Available unfold arrows on dashboard open while still allowing Right Arrow on a focused unknown Available row to ask Pi to inspect/cache resources.
- Match equivalent package source spellings such as `https://github.com/owner/repo` and `git:https://github.com/owner/repo` so loaded/installed packages do not split into misleading Available/Unloaded rows.
- Clean up package child resource rows by showing Pi folder-style entrypoints as folders while preserving/noting the exact `index.ts`, `index.js`, or `SKILL.md` filter path.
...
- Clarify dashboard remove behavior around package child rows: `r` remains whole-package project removal, while child resources are filtered rather than removed.
