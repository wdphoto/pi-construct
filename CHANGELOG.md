...
- Updated the `/construct` dashboard row grammar to separate selection from state: `[x]` marks selected rows, while compact `✓`, `–`, `+`, and `◇` icons plus row colors show current state.
