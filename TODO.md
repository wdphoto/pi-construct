...
- Done: `/construct run <saved-name>` now uses project inventory to skip already-active sources and enable disabled matching declarations instead of scheduling every saved source as `Install`.
- Done: Available package child-resource picking re-resolves after install before writing filters and warns when the cached temporary resource list differs from the installed package.
- Done: package resource filter write planning moved into `package-resource-plans.ts` with smoke coverage for same relative path across resource kinds and all-empty selections.
- Done: `/construct unload <source>` now uses package source identity matching for query selection, current-project active counts, and saved-loadout source cleanup.
