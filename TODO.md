...
- Should autoload keep the session-time `.pi/settings.json` watcher, or should it simplify to exit-time-only prompts?
...
- If Pi exposes a stable package-install or settings-change event later, consider replacing filesystem watching instead of expanding it.
