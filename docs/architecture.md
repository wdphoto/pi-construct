...
3. **Package operation layer**
   - Loads available sources with Pi's native project-local install path:
     ...
     ```
   - For Available package child-resource selection, lazily inspects remembered sources with Pi's temporary resolver when the user presses Right Arrow on an Available row, then installs project-local and writes package filters for the selected resources.
