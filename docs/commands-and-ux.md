...
- `/construct unload <id-or-source ...>` removes matching library resources directly. Source queries use the same package source identity matching as load/dashboard, so equivalent local paths and git/npm spellings match the remembered resource and matching saved-loadout source references are cleaned up.
...
- Running a saved loadout is activate-only: it skips already-active recipe package sources, enables disabled recipe package sources, and installs missing/available recipe package sources. It does not disable/remove active packages, remove packages outside the recipe, or exact-match the project to the recipe.
...
- row grammar separates selection from state: `[x]` means selected/all package resources, `[~]` means mixed package resources, `[-]` means all visible package resources targeted off, `[·]` means included by the focused loadout recipe, `[!]` means read-only, while compact icons `◆`, `✓`, `–`, `+`, or `◇` describe current state; section headings carry the state words;
- Right Arrow on a package row unfolds package-contained resources inline using Pi's native resolver when the package has multiple resolved resources; Left Arrow folds. Parent rows summarize child-resource state with `[~]`, `[x]`, or `[-]`; Space on `[~]` selects all child resources, Space on `[x]` turns all child resources off, and Space on `[-]` returns to the original child-resource targets so there is no pending package-resource change. Child rows show explicit `on`/`off` target state, with `[x]` on a child meaning that child target changed. Enter previews/writes native Pi package filters. `Available` package rows are cache-inspected without network/download during dashboard build and show the normal collapsed arrow only when multiple resources are already known; if no cached multi-resource list is known, the row has no hidden Right Arrow action and Enter installs the whole package with Pi defaults. Available child selections start unchecked, install the package project-local first, re-resolve the installed package resources, warn if the cache differed, and then narrow it with native Pi package filters. Resource-level selection writes an explicit allowlist across all package resource kinds so future package-added resources stay disabled until selected; whole-package row Enter is the path for trusting package defaults. Single-resource and unknown packages stay as whole-package rows and show no unfold affordance. Folder-style Pi entrypoints are displayed as folders, with their exact `index.ts`, `index.js`, or `SKILL.md` path noted in row details;
...
- show compact package source labels in rows where possible, e.g. `github:owner/repo` or `local:<dir>`, while keeping exact source strings in saved metadata and write operations; equivalent npm/git/local source spellings are matched as one package for dashboard state, load adoption, and filter writes;
...
- keep the footer short and two-line: controls first, then `[~] mixed resources · [-] all resources off · [!] read-only · [·] recipe item`;
...
- package disable/enable remains a whole-package toggle for unfiltered or whole-package-disabled packages. Construct now skips selected parent package rows when a package already has partial Pi package filters, rather than running into the guard or silently clobbering resource-level selections; package-row child selection is the native Pi-filter path for intentional partial changes;
