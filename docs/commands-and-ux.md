...
- Right Arrow on a package row unfolds package-contained resources inline using Pi's native resolver when the package has multiple resolved resources; Left Arrow folds. Space on child rows changes target enabled state, and Enter previews/writes native Pi package filters. `Available` package rows are cache-inspected without network/download during dashboard build and show the normal collapsed arrow only when multiple resources are already known; if no cached list is known, the focused row can still use Right Arrow to ask Pi to inspect/cache the package without showing a false unfold arrow first. Available child selections start unchecked, install the package project-local first, and then immediately narrow it with native Pi package filters. Resource-level selection writes an explicit allowlist across all package resource kinds so future package-added resources stay disabled until selected; whole-package row Enter is the path for trusting package defaults. Single-resource and unknown-after-inspection packages stay as whole-package rows and show no unfold affordance. Folder-style Pi entrypoints are displayed as folders, with their exact `index.ts`, `index.js`, or `SKILL.md` path noted in row details;
