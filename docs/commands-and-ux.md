...
- row grammar separates selection from state: `[x]` means selected/all, `[~]` means mixed child state with no child selection, `[-]` means active children selected, `[+]` means inactive/available children selected, `[*]` means a custom child selection, `[·]` means included by the focused loadout recipe, and `[!]` means read-only; compact icons `◆`, `✓`, `–`, `+`, or `◇` describe current state;
- Right Arrow on a package row unfolds package-contained resources inline using Pi's native resolver when the package has multiple resolved resources; Left Arrow folds. Child state icons show current state (`✓` active, `–` inactive, `+` available), while `[x]` selects a child for the next action. Space on a package row cycles child selection presets: all → active → inactive/available → none. Enter previews/writes native Pi package filters: selected existing children are toggled, unselected existing children keep their current state, and selected Available children are installed/enabled. `Available` package rows are cache-inspected without network/download during dashboard build and show the normal collapsed arrow only when multiple resources are already known; if no cached multi-resource list is known, the row has no hidden Right Arrow action and Enter installs the whole package with Pi defaults. Resource-level selection writes an explicit allowlist across all package resource kinds so future package-added resources stay disabled until selected; whole-package row Enter is the path for trusting package defaults. Single-resource and unknown packages stay as whole-package rows and show no unfold affordance. Folder-style Pi entrypoints are displayed as folders, with their exact `index.ts`, `index.js`, or `SKILL.md` path noted in row details;
...
- keep the footer short and two-line: controls first, then parent Space cycle and compact marker meanings such as `[~] mixed state`, `[-] active selected`, `[+] inactive/available selected`, `[*] custom selection`, `[!] read-only`, and `[·] recipe item`;
