...
- color-code the visible row by state so scanning does not depend on the icon alone: Installed uses Pi's default success green, Disabled uses muted green, Available uses warning yellow, and Unloaded uses muted gray;
