...
- Right Arrow unfolds Active/Disabled package resources only when multiple resources are known; `i` lazily inspects Available package resources when Pi can resolve them; Available child selections start unchecked and install project-local with native filters, while Active/Disabled child selections only update filters;
