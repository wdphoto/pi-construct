...
- Right Arrow unfolds Active/Disabled package resources only when multiple resources are known, and lazily inspects Available package rows when Pi can resolve them; Available child selections start unchecked and install project-local with native filters, while Active/Disabled child selections only update filters; child-resource selections are explicit allowlists so future package-added resources stay disabled until selected;
