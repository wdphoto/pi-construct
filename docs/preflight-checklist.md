...
- Right Arrow unfolds package resources when multiple resources are already known; child state icons show current state (`✓` active, `–` inactive, `+` available), while `[x]` selects a child for the next action; parent package rows show `[~]` for mixed child-resource state with nothing selected, `[x]` for all children selected, `[-]` for active children selected, `[+]` for inactive/available children selected, and `[*]` for a custom child selection; Space on a parent cycles all → active → inactive/available → none and collapse/reopen preserves selection; Available package rows are cache-inspected without network/download during dashboard build, unknown Available rows with no cached multi-resource list have no hidden Right Arrow action, and single-resource or unknown packages stay whole-package rows; selected existing child resources are toggled by native filters, unselected existing children keep their current state, and selected Available children install project-local with native filters; child-resource selections are explicit allowlists so future package-added resources stay disabled until selected;
