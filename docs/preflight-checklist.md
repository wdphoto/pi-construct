...
- Right Arrow unfolds package resources when multiple resources are already known; Available package rows are cache-inspected without network/download during dashboard build, and focused unknown Available rows can use Right Arrow to ask Pi to inspect/cache resources without showing a false unfold arrow first; single-resource or unknown-after-inspection packages stay whole-package rows; Available child selections start unchecked and install project-local with native filters, while Active/Disabled child selections only update filters; child-resource selections are explicit allowlists so future package-added resources stay disabled until selected;
