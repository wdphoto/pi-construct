...
- Right Arrow unfolds package resources only when multiple resources are already known; Available package rows are cache-inspected without network/download during dashboard build, and single-resource or unknown packages stay whole-package rows; Available child selections start unchecked and install project-local with native filters, while Active/Disabled child selections only update filters; child-resource selections are explicit allowlists so future package-added resources stay disabled until selected;
