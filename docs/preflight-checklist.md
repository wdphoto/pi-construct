...
- Right Arrow unfolds package resources when multiple resources are already known; parent package rows show `[~]` for mixed child-resource state, `[x]` for all child resources targeted on, and `[-]` for all child resources targeted off; Space on `[~]` selects all child resources, Space on `[x]` turns all child resources off, and Space on `[-]` returns to the original child-resource targets/no pending package-resource change; Available package rows are cache-inspected without network/download during dashboard build, unknown Available rows with no cached multi-resource list have no hidden Right Arrow action, and single-resource or unknown packages stay whole-package rows; Available child selections start unchecked and install project-local with native filters, while Active/Disabled child selections only update filters; child-resource selections are explicit allowlists so future package-added resources stay disabled until selected;
