...
- A saved loadout is a named grouping of **active Construct resources**.
- Disabled resources are skipped.
- Active project resources that are not loaded into Construct are offered during save; selected resources are loaded into Construct and included, unselected resources are skipped.
...
> “What active Construct resources make up this workflow?”

It is intentionally not a full `.pi/settings.json` snapshot. The unit is still a Pi package source remembered by Construct. Pi remains responsible for package installation, package resolution, project trust, resource execution, updates, and reloads.
...
Construct reads the current project state and classifies package declarations:
...
These active project resources are not loaded into Construct yet.
...
Included: 3 resources
Loaded into Construct and included: 1
Skipped active resources not loaded into Construct: 1
Skipped disabled resources: 2
```
...
In non-TUI mode, replacement should refuse for now because there is no confirmation UI. Non-TUI save may still create a new saved loadout and should report skipped active resources that were not already loaded into Construct.
...
- Include active resources only.
- No secrets, auth material, env values, generated package cache paths, or installed cache locations.
- Local path sources may be saved for personal reuse but should be warned as not generally shareable across machines.
...
```text
Loadout: 2 installed | 1 disabled | 3 available | 0 unloaded

Saved
-----
[ ] ◆  web-stack       4 resources
[ ] ◆  pi-projects     3 resources
...
- Saved-loadout row grammar: `[ ] ◆  web-stack  4 resources`.
...
- Update save behavior to include active Construct-managed resources and offer active unloaded project resources for inclusion in TUI.
...
- Add `/construct copy [saved-name]` to print a JSON snippet for a saved loadout or current active Construct grouping.
