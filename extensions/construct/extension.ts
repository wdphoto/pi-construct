...
function getRemovablePackageSources(settings: JsonReadResult): string[] {
	const sources: string[] = [];
...
function updateConstructSourcesEnabled(construct: JsonReadResult, sources: Set<string>, enabled: boolean): { data: JsonObject; changed: number } {
	const root = parseProjectConstruct(construct);
	const items = isObject(root.items) ? root.items : {};
...
async function handleUnloadAll(pi: ExtensionAPI, ctx: ExtensionCommandContext, paths: ConstructPaths): Promise<void> {
